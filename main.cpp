...
bool tryInsertFromInput(Tree23& tree, VisualizerUI& ui, std::vector<Tree23Node*>& highlightPath,
                        bool& searchFound, std::vector<Tree23Node*>& fullSearchPath,
...
void resetAnimation(std::vector<Tree23Node*>& highlightPath,
                    std::vector<Tree23Node*>& fullSearchPath, int& currentAnimationStep,
                    bool& isPlaying, bool& searchFound, VisualizerUI& ui) {
    highlightPath.clear();
    fullSearchPath.clear();
    currentAnimationStep = -1;
    isPlaying = false;
...
void resetInsertAnimation(std::vector<Tree23Step>& insertSteps, int& currentInsertStep,
                          bool& isInsertPlaying, int& pendingInsertValue, VisualizerUI& ui,
...
void resetDeleteAnimation(std::vector<Tree23Step>& deleteSteps, int& currentDeleteStep,
                          bool& isDeletePlaying, int& pendingDeleteValue, VisualizerUI& ui,
                          const std::string& speedLabel) {
    deleteSteps.clear();
    currentDeleteStep = -1;
                ...
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (isButtonClicked(ui.insertButton, mousePos)) {
                        ...
                        if (isValid == false) {
                            ...
                            resetAnimation(highlightPath, fullSearchPath, currentAnimationStep,
                                           isPlaying, searchFound, ui);

                            resetDeleteAnimation(deleteSteps, currentDeleteStep, isDeletePlaying,
                                                 pendingDeleteValue, ui, speedLabel);

                            resetInsertPathState(currentHighlightedNode, insertPath);
                            resetInsertAnimation(insertSteps, currentInsertStep, isInsertPlaying,
                                                 pendingInsertValue, ui, speedLabel);
                            ...
                            if (!insertSteps.empty()) {
                                updateAnimationText(ui, "Animation: " + insertSteps[0].action,
                                                    speedLabel);
                            } else {
                                updateAnimationText(ui, "Animation: idle", speedLabel);
                            }
                    ...
                    if (isButtonClicked(ui.deleteButton, mousePos)) {
                        ...
                        if (isValid == false) {
                            ...
                            if (!deleteSteps.empty()) {
                                updateAnimationText(ui, "Animation: " + deleteSteps[0].action,
                                                    speedLabel);
                            } else {
                                updateAnimationText(ui, "Animation: idle", speedLabel);
                            }
                    ...
                    if (isButtonClicked(ui.playButton, mousePos)) {
                        if (!insertSteps.empty() && currentInsertStep >= 0 &&
                            currentInsertStep + 1 < (int)insertSteps.size()) {
                            isInsertPlaying = true;
                            updateAnimationText(ui, "Animation: playing insert", speedLabel);
                            animationClock.restart();
                        } else if (!deleteSteps.empty() && currentDeleteStep >= 0 &&
                                   currentDeleteStep + 1 < (int)deleteSteps.size()) {
                            isDeletePlaying = true;
                            updateAnimationText(ui, "Animation: playing delete", speedLabel);
                            animationClock.restart();
                        } else if (!fullSearchPath.empty() &&
                                   currentAnimationStep + 1 < (int)fullSearchPath.size()) {
                            isPlaying = true;
                            updateAnimationText(ui, "Animation: playing", speedLabel);
                            animationClock.restart();
                        }
        ...
        if (isInsertPlaying) {
            if (animationClock.getElapsedTime().asSeconds() >= animationDelay) {
                ...
                if (currentInsertStep + 1 < (int)insertSteps.size()) {
                    currentInsertStep++;
                    updateAnimationText(ui, "Animation: " + insertSteps[currentInsertStep].action,
                                        speedLabel);
        ...
        if (isDeletePlaying) {
            if (animationClock.getElapsedTime().asSeconds() >= animationDelay) {
                ...
                if (currentDeleteStep + 1 < (int)deleteSteps.size()) {
                    currentDeleteStep++;
                    updateAnimationText(ui, "Animation: " + deleteSteps[currentDeleteStep].action,
                                        speedLabel);
                } else {
